...
/**
 * Per-table availability summary for a specific date, used by the member
 * booking flow to colour the floorplan for the selected date (rather than
 * "right now"). Generated by {@link getTableAvailabilityForDate}.
 */
export interface TableDateAvailability {
  id: string;
  table_number: number;
  name: string;
  /** How many 1-hour slots are bookable on this date. */
  available_slots: number;
  /** Total possible slots (close_hour - open_hour). */
  total_slots: number;
  /** Simplified status for floorplan colouring. */
  status: "available" | "limited" | "full" | "blocked";
  /** Whether the current member has a confirmed booking on this table on this date. */
  member_has_booking: boolean;
}
...
// ---------- Date-specific table availability (member booking flow) ----------

/**
 * Returns a summary of each table's availability on the given YYYY-MM-DD
 * date. Used by the member booking flow to paint the floorplan with
 * date-specific statuses instead of the "right now" view.
 *
 * Counts 1-hour bookable slots via {@link getAvailableSlots} (which handles
 * both mock and Supabase paths) and tags each table with `member_has_booking`
 * when the supplied member already has a confirmed session on that table
 * overlapping the selected day.
 */
export async function getTableAvailabilityForDate(
  date: string,
  memberId: string | null
): Promise<TableDateAvailability[]> {
  ...
  const memberBookingTableIds = memberId
    ? await fetchMemberBookingTableIds(memberId, dayStartIso, dayEndIso)
    : new Set<string>();

  const totalSlots = VENUE_CLOSE_HOUR - VENUE_OPEN_HOUR;

  const results = await Promise.all(
    tables.map(async (t) => {
      const slots = await getAvailableSlots(t.id, date);
      const availableSlots = slots.filter((s) => s.available).length;

      // Past slots are never bookable and shouldn't cause us to classify a
      // table as "blocked" just because the morning has already elapsed.
      const nonPastSlots = slots.filter((s) => s.reason !== "Past");
      const allNonPastBlocked =
        nonPastSlots.length > 0 &&
        nonPastSlots.every((s) => !s.available && s.reason === "Blocked");

      let status: TableDateAvailability["status"];
      if (availableSlots === 0 && allNonPastBlocked) {
        status = "blocked";
      ...
      return {
        ...
        status,
        member_has_booking: memberBookingTableIds.has(t.id),
      };
    })
  );

  // Preserve table_number ordering so the caller doesn't need to re-sort.
...
async function fetchMemberBookingTableIds(
  memberId: string,
  dayStartIso: string,
  dayEndIso: string
): Promise<Set<string>> {
  if (!isSupabaseConfigured()) {
    const ids = MOCK_BOOKINGS.filter(
      (b) =>
  ...
  const { data } = await supabase
    ...
    .lt("starts_at", dayEndIso)
    .gt("ends_at", dayStartIso);
  const rows = (data as { table_id: string }[] | null) ?? [];
